...
{	
	if (moverComponent && OtherActor && OtherActor->ActorHasTag("PressurePlateActivator"))
	{
		numOfActivators++; // Increment the number of activators overlapping the trigger
...
void UTriggerComponent::OnOverlapEnd(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	if (moverComponent && OtherActor && OtherActor->ActorHasTag("PressurePlateActivator"))
	{
		numOfActivators--; // Decrement the number of activators overlapping the trigger
		numOfActivators = FMath::Max(0, numOfActivators); // Ensure the number of activators doesn't go below zero

		if (isTriggerActivated && numOfActivators == 0)
		{
